module.exports = function ({ api, models, Users, Threads, Currencies }) {
  ...
  return async function ({ event }) {
    ...
    const usedPrefix = threadSetting.hasOwnProperty("PREFIX") ? threadSetting.PREFIX : prefix;
    const prefixRegex = new RegExp(`^(<@!?${senderID}>|${escapeRegex(usedPrefix)})\\s*`);
    ...
    if (!command) {
      ...
      if (checker.bestMatch.rating >= 0.8) {
        ...
        api.sendMessage(`ها ${commandName} \n '${closestMatch}'?`, threadID, messageID);
        return;
      }
    }
